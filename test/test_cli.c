...
void test_version() {
  ...
  printf(">%s Running %s with --version\n", CLEAR, argv0[0]);
  int status0 = process_args(2, argv0);
  TEST_ASSERT_EQUAL_INT8(0, status0);

  printf(">%s Running %s with -v\n", CLEAR, argv0[0]);
  int status1 = process_args(2, argv1);
  TEST_ASSERT_EQUAL_INT8(0, status1);

  printf(">%s Running %s with --version with extra args\n", CLEAR, argv0[0]);
...
void test_config() {
  ...
  printf(">%s Running %s with --config\n", CLEAR, argv0[0]);
  int status0 = process_args(3, argv0);
  TEST_ASSERT_EQUAL_INT8(0, status0);

  printf(">%s Running %s with -c\n", CLEAR, argv0[0]);
  int status1 = process_args(3, argv1);
  TEST_ASSERT_EQUAL_INT8(0, status1);

  printf(">%s Running %s with --config without a file\n", CLEAR, argv0[0]);
  int status2 = process_args(3, argv2);
  TEST_ASSERT_EQUAL_INT8(1, status2);
}

void test_incorrect_flag() {
  char *argv0[2] = {"hsh", "--gabagool"};

  printf(">%s Running %s with an incorrect flag\n", CLEAR, argv0[0]);
  int status0 = process_args(2, argv0);
  TEST_ASSERT_EQUAL_INT8(0, status0);
}
